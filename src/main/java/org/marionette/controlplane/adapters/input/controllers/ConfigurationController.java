...
public class ConfigurationController {
    ...
    @Autowired
    public ConfigurationController(ConfigRegistry configRegistry) {
    ...
    /**
     * PUT /api/services/{serviceName}/classes/{className}/methods/{methodName}/behavior
     * This is called when user changes a behavior in the React UI
     */
    @PutMapping("/services/{serviceName}/classes/{className}/methods/{methodName}/behavior")
    public ResponseEntity<String> updateMethodBehavior(
            @PathVariable String serviceName,
            @PathVariable String className,
            @PathVariable String methodName,
            @RequestBody BehaviourChangeRequest request) {
        
        try {
            ServiceName service = new ServiceName(serviceName);
            ClassName clazz = new ClassName(className);
            MethodName method = new MethodName(methodName);
            BehaviourId newBehavior = new BehaviourId(request.getNewBehaviourId());
    ...
    private void notifyMarionetteNodes(String serviceName, String className, String methodName, BehaviourId newBehavior) {
        System.out.println("Notifying marionette nodes: " + serviceName + "." + className + "." + methodName + " -> " + newBehavior);
    }
}
